...
//    10번 이상 상세 페이지를 조회했는지 검사
    public boolean isOkayToRetrieve(Long memberId){
        System.out.println("다오입니다: "+memberId+"sout");
        int count = retrievedExperienceMapper.selectCountRetrieved(memberId);
        System.out.println("조회 횟수: "+count+"sout");
        boolean result = count > 9;
        return result;
    }
}
